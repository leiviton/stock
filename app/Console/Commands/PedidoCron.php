...
WHERE SC7.D_E_L_E_T_='' AND SC7.C7_CONAPRO <> 'L' AND C7_XENVEML <> 1
					AND SC7.C7_EMISSAO >= '20200217' 
				GROUP BY SC7.C7_NUM,
							SC7.C7_NUMSC,
							SC1.C1_NOMESOL,
							SC7.C7_DATPRF,
        ...
        //dd($arraySolicitationNumber);AND C7_XENVEML <> 1
        //dd($solicitUnique);
...
FROM SC7010 SC7
			LEFT JOIN SC1010 SC1 ON (SC1.D_E_L_E_T_='' AND SC1.C1_FILIAL = SC7.C7_FILIAL AND SC1.C1_NUM = SC7.C7_NUMSC)
WHERE SC7.D_E_L_E_T_='' AND SC7.C7_CONAPRO <> 'L' AND C7_XENVEML <> 1 AND C7_NUMSC = $solicitUnique[$j]
					AND SC7.C7_EMISSAO >= '20200217'
				GROUP BY SC7.C7_NUM,
							SC7.C7_NUMSC,
							SC1.C1_NOMESOL,
